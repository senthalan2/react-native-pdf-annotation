...
import static com.pdfannotation.PdfAnnotationModule.reactContext;
...
//			mOutlineButton.setVisibility(View.INVISIBLE);
	...
	public void requestPassword(final Bundle savedInstanceState) {
		...
		alert.setButton(AlertDialog.BUTTON_NEGATIVE, getString(com.pdfannotation.R.string.cancel),
				new DialogInterface.OnClickListener() {
	...
	private void sendPageNumber(int pageNumber, int totalPages){
		WritableMap payload = Arguments.createMap();
		// Put data to map
		payload.putInt("pageNumber", pageNumber);
		payload.putInt("totalPages", totalPages);
		reactContext
				.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class)
				.emit("onPdfPageChange", payload);
	}
	...
	public void createUI(Bundle savedInstanceState) {
		...
		mDocView = new MuPDFReaderView(this) {
			...
			protected void onMoveToChild(int i) {
				if (core == null)
					return;
				sendPageNumber(i+1,core.countPages());
...
//		});
		...
		mSearchBack.setColorFilter(Color.argb(255, 220, 220, 220));
		mSearchFwd.setColorFilter(Color.argb(255, 220, 220, 220));
...
//		setButtonEnabled(mLinkButton, !reflow);
	...
	private void setButtonEnabled(ImageButton button, boolean enabled) {
		...
		button.setColorFilter(enabled ? Color.argb(255, 255, 255, 255) : Color.argb(255, 220, 220, 220));
	}
...
//		mLinkButton.setColorFilter(highlight ? Color.argb(0xFF, 172, 114, 37) : Color.argb(0xFF, 255, 255, 255));
		...
		if (!mButtonsVisible) {
			...
			anim.setAnimationListener(new Animation.AnimationListener() {
				public void onAnimationStart(Animation animation) {
					...
					mFilenameView.setFocusable(true);
					mFilenameView.setText(mFileName);
				}
	...
	private void updatePageNumView(int index) {
		if (core == null)
			return;
		sendPageNumber(index + 1,core.countPages());
    ...
    Boolean isEnableCustomFooterColor = false;
      ...
      try{
        isEnableCustomFooterColor = extras.getBoolean("isEnableCustomFooterColor");
      }catch(Exception e){}
...
//		mTopBarMode = TopBarMode.More;
	...
	public void onBackPressed() {
		if (core != null && core.hasChanges()) {
			DialogInterface.OnClickListener listener = new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					if (which == AlertDialog.BUTTON_POSITIVE){
						core.save();
						if (load != null) {
							Log.v("Notification", "Load is not null");
							load.encrypt();
						} else {
							Log.v("Notification", "Load is null");
							Uri uri = getIntent().getData();
							load = new LoadFile(MuPDFActivity.this, mFileName, uri.getPath());
							load.encrypt();
						}
						int index = mDocView.getDisplayedViewIndex();
            WritableMap payload = Arguments.createMap();
            // Put data to map
            payload.putString("filePath", load.getSourceFilePath());
            payload.putInt("currentPage", index+1);
            payload.putInt("totalPages", core.countPages());
            reactContext
              .getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class)
              .emit("onCloseWithSave", payload);
					}
					else{
						int index = mDocView.getDisplayedViewIndex();
            WritableMap payload = Arguments.createMap();
            ...
            reactContext
              .getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class)
              .emit("onCloseWithoutSave", payload);
					}
					finish();
